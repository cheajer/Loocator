...
export default class DisplayReviews extends Component {
    ...
    getTitle = (title, review_score) => {
        return <div style={{alignItems:'center'}}><Typography>{title}</Typography><Rating precision={0.1} readOnly value={review_score}/></div>
    }
    ...
    render() {
        ...
        return (    
            ...
            <List className="" style={{padding:0}}>
                {this.mapStructure(rows.slice((this.state.page-1) * 3, (this.state.page-1) * 3 + 3))}
            </List>
